...
function App() {
  ...
  const handleProcessClick = async () => {
    if (selectedImage) {
      ...
      await axios.post('http://192.168.1.14:8000/enhance_image', formData).then((data) => {
  ...
  const handleDecompressClick = async () => {
    if (compressedImage) {
      ...
      await axios.post('http://192.168.1.14:8000/decompress_image', { compressed_img: compressedImage }).then((data) => {
